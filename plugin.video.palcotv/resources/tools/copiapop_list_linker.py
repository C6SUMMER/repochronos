# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Linker de PlayLists de OpenLoad by DMax
...
fanart = 'http://i64.tinypic.com/2s69a11.png'
thumbnail = 'http://i66.tinypic.com/2em21av.png'


def copiapop_list_linker(busqueda):

	if busqueda.find("gallery") < 0:
		if busqueda.find("/list,") >= 0:
			busqueda = busqueda.replace("/list,", "/gallery,")
		else:	
			lista = busqueda +"/gallery,1,1"
	else:
		lista = busqueda
	...
	plugintools.add_item(action="",url="",title="[COLOR yellow][I]              ····[COLOR skyblue]Linker PlayLists de CopiaPop[COLOR yellow]····[COLOR red][B]      (by DMax)[/B][/I][/COLOR]",thumbnail=thumbnail,fanart=fanart,folder=False,isPlayable=False)
	...
	nom_canal = plugintools.find_single_match(busqueda, "http://copiapop.com/(.*?)/")
	canal = "http://copiapop.com/"+nom_canal
	salir = False
	...
	plugintools.add_item(action="openload_canal_listas",url=canal,title="[COLOR white]Ver Resto de Listas del Canal: [COLOR red]"+nom_canal+"[/COLOR]",thumbnail=thumbnail,fanart=fanart,folder=True,isPlayable=False)
		
		
		
		
def openload_canal_listas(params):
	...
	nom_canal = plugintools.find_single_match(canal, "http://copiapop.com/(.*)")
	...
	todas_listas = plugintools.find_single_match(data, '<div class="collections_list responsive_width">(.*?)<div class="collections_list responsive_width followed_list" id="followed_collections">')
	...
	for item in cada_lista:
		...
		busqueda = "http://copiapop.com" + url_lista +"/gallery,1,1"
		plugintools.add_item(action="pop_lanza_lista",url=busqueda,title="[COLOR white]Ver Lista: [COLOR red]"+titulo+"[/COLOR]",thumbnail=logo,fanart=fanart,folder=True,isPlayable=False)
	
	
	
def pop_lanza_lista(params):
	busqueda = params.get("url")
	
	copiapop_list_linker(busqueda)
