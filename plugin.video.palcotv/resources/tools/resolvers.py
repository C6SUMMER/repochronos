...
def uptostream(params):
    ...
    try:
        ...
        if "Video not found" in data:
            xbmc.executebuiltin("Notification(%s,%s,%i,&s)" % ('PalcoTV', "Archivo borrado", 3 , art+'icong.png'))
        else:
            try:
...
def youtube(params):
    ...
    try:
        ...
        try:
            ######## Utilizamos el addon de youtube como primera opcion ###########
...
def thevideome(params):
    ...
    try:
        ...
        r = requests.get(page_url)
        data = r.content
        key_tkn = plugintools.find_single_match(data,"var tkn='(.*?)'")
...
def thevideobee(params):
    ...
    #try:
    print page_url 
    r = requests.get(page_url,headers=headers)#,headers=headers)
    #data = r.text
    #print data
    #data = plugintools.find_single_match(data,"eval\(function\(p,a,c,k,e,d\)(.*?)</script>")
    #data = wiz.unpack(data)
...
def yourvideohost(params):
    ...
    try:
        ...
        'Referer': page_url}
        print post
        r = requests.get(post_url,data=post)
        data = r.text
        print data
        try:
            media_url = plugintools.find_single_match(data,'file: "([^"]+)"')
